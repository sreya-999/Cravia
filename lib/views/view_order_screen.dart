...
// Display the total quantity with proper text
    String quantityText = "$totalQuantity ${totalQuantity == 1 ? 'Item' : 'Items'} in cart";
                                                ...
                                                Expanded(
                                                  child: Column(
                                                    ...
                                                    children: [
                                                      ...
                                                      // Text(
                                                      //   (item.addOnNames != null && item.addOnNames!.isNotEmpty)
  ...
  Widget _buildQtyButton(BuildContext context, IconData icon) {
    ...
    return Container(
      padding: EdgeInsets.symmetric(
          horizontal: isTablet ? 10 : 7, vertical: isTablet ? 10 : 7),
      decoration: BoxDecoration(
        borderRadius: BorderRadius.circular(12),
...
//   }
  ...
  Widget _priceRow(String label, BuildContext context, String value,
      {bool isBold = false}) {
    final responsive = Responsiveness(context);
    final double screenWidth = MediaQuery.of(context).size.width;
    final bool isDesktop = screenWidth >= 1024;
        ...
        return Align(
          ...
          child: Material(
            ...
            child: Padding(
              ...
              child: Container(
                width: screenWidth,
                height: isTablet ? screenHeight * 0.3 : screenHeight * 0.5,
                padding: const EdgeInsets.all(16),
                decoration: const BoxDecoration(
                  gradient: const LinearGradient(
                    colors: [AppColor.secondary, AppColor.primaryColor],
  ...
  //
  //   double total = selectedProvider.getChildCategoryOrDiscountTotal(
  //       product, selectedProvider.selectedChildCategory, selectedProvider);
  //   print('Final Total: $total');
  //
  //   selectedProvider.setSelectedChildCategoryId(
  //     product.id,
  //     product.childCategoryId.toString(),
  //   );
  //
  //   // Check if product is already in cart and get current quantity
  //   // final cartItem = cartProvider.getCartItemById(product.id, childCategoryId: selectedProvider.selectedChildCategory?.id.toString());
  //   //
  //   // if (cartItem != null) {
  ...
  //                                                           CategoryProvider>();
  //
  //                                                   final selectedChildCategoryId =
  //                                                       provider
  //                                                           .getSelectedChildCategoryId(
  //                                                               product.id);
  //
  //                                                   // ✅ Print the selected category ID
  //                                                   print(
  //                                                       "Selected Child Category ID for product ${product.id}: $selectedChildCategoryId");
  //
  ...
  //                                               print(
  //                                                   'TakeAway Price: ${provider.takeAwayPrice}');
  //                                               print(
  //                                                   'Total Combo Price: $totalComboPrice');
  //
  //                                               return ShaderMask(
  //                                                 shaderCallback: (bounds) =>
  ...
  //                                                     product.categoryId,
  //                                                 price: isTakeAway
  //                                                     ? ((double.tryParse(product.discountPrice?.toString() ?? '0') ??
  //                                                             0.0) +
  //                                                         (double.tryParse(product.takeAwayPrice?.toString() ?? '0') ??
  //                                                             0.0))
  //                                                     : (selectedProvider
  //                                                             .selectedPrices ??
  //                                                         0.0),
                                ...
                                Container(
                                  ...
                                  decoration: const BoxDecoration(
                                    gradient: const LinearGradient(
                                      ...
                                      begin: Alignment
                                          .topCenter, // Start at the very top
                                      end: Alignment
                          ...
                          child: ElevatedButton(
                            onPressed: () {
                              ...
                              final selectedChild = context
                                  .read<CategoryProvider>()
                                  .selectedChildCategory;
                              print("Images in product: ${product.images}");
                              final totalTime =
                                  context.read<CategoryProvider>().totalTime;
                              final cartProvider = Provider.of<CartProvider>(
                                  context,
                              ...
                              WidgetsBinding.instance.addPostFrameCallback((_) {
                                if (context.mounted) {
                                  PopupDialog.show(
                                      context, product.disountPercent);
                                }
                              });
                            },
                            style: ElevatedButton.styleFrom(
